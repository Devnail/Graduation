...
public class OrderSqlProvider {
    ...
    public String count() {
        return "SELECT count(*) FROM t_yw_order WHERE DELETE_STATUS=0";
    }

    public String countByUserId() {
        return "SELECT count(*) FROM t_yw_order WHERE USER_ID=#{userId} AND DELETE_STATUS=0";
    }
    ...
    public String state0count() {
        return "SELECT count(*) FROM t_yw_order WHERE DELETE_STATUS=0 AND PRODUCT_TYPE=0";
    }

    public String state1count() {
        return "SELECT count(*) FROM t_yw_order WHERE DELETE_STATUS=0 AND PRODUCT_TYPE=1";
    }

    public String state2count() {
        return "SELECT count(*) FROM t_yw_order WHERE DELETE_STATUS=0 AND PRODUCT_TYPE=2";
    }
}
